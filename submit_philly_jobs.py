...
parser.add_argument('--name', default='adv_dcl_2_500_200_1023')
parser.add_argument('--main', default='train_v2.py')
parser.add_argument('--vc', default='resrchvc')
parser.add_argument('--data_dir', default='squad_at/docker', help='based on user root')
parser.add_argument('--model_dir', default='squad_at/docker/adv_dcl_2_500_200_1023', help='based on user root')
parser.add_argument('--seed', default = 1023)
parser.add_argument('--train_data', default='train_data_v2_adv.json')
parser.add_argument('--dev_data', default='dev_data_v2_adv.json')
parser.add_argument('--meta', default='squad_meta_v2_adv.pick')
parser.add_argument('--cluster', default='gcr')
...
with open(args.template) as f:
    ...
    for line in f:
        ...
        write_line = write_line.replace('SEED', str(args.seed)) 
        write_line = write_line.replace('TRAIN_DATA', args.train_data)
        write_line = write_line.replace('DEV_DATA', args.dev_data)
        write_line = write_line.replace('META', args.meta)
